...
def run_game():
    ...
    screen = pygame.display.set_mode((Bf_settings.screen_width, Bf_settings.screen_height))
    pygame.display.set_caption("Air Craft Battle Game")


    # Make a ship
    ship = Ship(screen, Bf_settings)
    ...
    while Bf_settings.gameInterface:
        gf.check_events_settings(Bf_settings)
        screen.blit(Bf_settings.image, (0, 0))
        pygame.display.flip()

    gf.change_screen(Bf_settings)
    ...
    while True:
        ...
        # pygame.display.flip()
        ship.update()
        gf.update_screen(ship, screen, Bf_settings.image)
